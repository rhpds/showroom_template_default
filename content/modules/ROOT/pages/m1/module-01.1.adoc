...
* Click on the devspaces.{ocp_cluster_openshift_cluster_ingress_domain}#/https://github.com/rpscodes/app_connectivity_summit25.git[Devspaces, window="devspaces"] link.

*Wait for the Dev Spaces workspace to initialize and come load. It usually takes a few seconds to a minute.

* It should open a IDE type interface. Click on the image:../assets/images/kebab.png[width=20,height=20] icon and open a *New Terminal* as shown below.
+
image::m1/new_terminal.png[]

* * Click on the bash icon and select rename. 
+
...
* Click on the image:../assets/images/split_icon.png[width=20,height=20] icon as shown in the image to create a new terminal, where you would be automatically logged into the OpenShift cluster.
+
