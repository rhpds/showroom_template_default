...
. In a browser window, navigate to the browser tab pointing to the OpenShift cluster. If you don't have a browser tab open on the console, click on {ocp_cluster_openshift_cluster_console_url}[OpenShift Console, window="console"] to launch the console. Login as `({ocp_cluster_openshift_cluster_admin_username}/{ocp_cluster_openshift_cluster_admin_password})` when prompted.
