...
. First lets try to access our travels service and see if all our microservices have access to the database. We'll use the inbuilt terminal to the right. 
+
image::m1/missing.png[]

. In the first terminal (top terminal), login into the OpenShift cluster using the below command.
+
...
. From the Openshift Terminal (top terminal), Run the following to check if the travels services and all other services are able to access the on-prem database.
+
[source,sh,role="execute",subs=attributes+]
----
...
. You will get a "Quote not Found" error indicating that the database is not accessible. Let's set up the Virtual Application Network using Red Hat Service Interconnect in the subsequent steps that help our services access the database on-premises.
...
. You might see a message similar to the one show below. This indicates that the Service Interconnect routers are being created. Wait a few seconds for the warning message to disappear and the site to be created.
+
