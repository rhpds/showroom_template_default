...
. Wait for a minute or two for the new environment variable to take effect, then from the OpenShift terminal (right terminal), run the following to check if the travels services and all other services are able to access the on-prem database.
+
[source,sh,role="execute",subs=attributes+]
----
