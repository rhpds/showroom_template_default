...
You can call the `discounts` service from `travels` by running the command:

+
[source,sh,role="execute",subs=attributes+]
----
oc rsh $(oc get pod -l app=travels -o jsonpath='{.items[0].metadata.name}') \
...
[NOTE]
====
You may need to adjust the time in the Kiali graph from *Last 1m* to a larger window of time so see the traffic from the above command.
====
...
[source,sh,role="execute",subs=attributes+]
----
oc rsh $(oc get pod -l app=flights -o jsonpath='{.items[0].metadata.name}') \
  curl http://discounts.travel-agency.svc.cluster.local:8000/discounts/hello-from-flights

oc rsh $(oc get pod -l app=insurances -o jsonpath='{.items[0].metadata.name}') \
  curl http://discounts.travel-agency.svc.cluster.local:8000/discounts/hello-from-insurances

oc rsh $(oc get pod -l app=cars -o jsonpath='{.items[0].metadata.name}') \
  curl http://discounts.travel-agency.svc.cluster.local:8000/discounts/hello-from-cars

oc rsh $(oc get pod -l app=hotels -o jsonpath='{.items[0].metadata.name}') \
...
Lets break down the above `AuthorizationPolicy`
```yaml
apiVersion: security.istio.io/v1
kind: AuthorizationPolicy
metadata:
...
== Test the *AthorizationPolicy* 

Now that you have applied the AthorizationPolicy, let's test to make sure the right services with the `travel-agency` namespace have the correct access to the `discounts` service by running the following commands in the terminal:
