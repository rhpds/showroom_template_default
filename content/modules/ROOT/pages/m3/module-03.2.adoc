...
. Access the Web App by clicking https://travels-blue-ui-partner-blue-portal.{ocp_cluster_openshift_cluster_ingress_domain}/[here^, console="access Web app"]
...
. Refresh the Web application or access it from https://travels-blue-ui-partner-blue-portal.{ocp_cluster_openshift_cluster_ingress_domain}/[here^, console="access Web app"]. You should see a *403 - Forbidden* error
+
image:m3/partner-blue-403.png[] 
...
. Navigate to {ocp_cluster_openshift_cluster_console_url}/k8s/cluster/operator.openshift.io%7ev1%7eConsole/cluster/console-plugins[Openshift Console .Plugins^, window="console"].  
...
. You would now notice that the `ingress-gateway-rlp-lowlimits` and `
prod-web-deny-all` are showing up as `Accepted (Not Enforced)`
+
