...
. Navigate to {ocp_cluster_openshift_cluster_console_url}/k8s/cluster/operator.openshift.io%7ev1%7eConsole/cluster/console-plugins[Openshift Console Plugins^, window="console"].  Login as admin/{ocp_cluster_openshift_cluster_admin_password} if prompted.
...
. A Gateway (based on Istio Gateway) with a wildcard hostname `*.travels.{ocp_cluster_openshift_cluster_ingress_domain}`, and a TLS Policy has already been created. 
. In the Overview section of ocp_cluster_openshift_cluster_console_url/kuadrant/overview[Connectivity Link^, window="console"] plugin menu, you can view the  `prod-web` Gateway and the TLS Policy that has been setup for you.
+
image:m3/gateway-tls-view.png[]
. If you accedd the `prod-web` Gateway, the  *YAML* tab will display the Gateway details
+
...
spec:
  parentRefs:
    - name: prod-web
      namespace: ingress-gateway
  hostnames:
    - echo.{ocp_cluster_workshop_main_domain}
  rules:
    - backendRefs:
        - name: echo-api
          port: 8080
